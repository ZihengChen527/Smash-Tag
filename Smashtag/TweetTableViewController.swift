...
class TweetTableViewController: UITableViewController, UITextFieldDelegate
{
    ...
    var searchText: String? {
        didSet {
            ...
            title = searchText
            if searchText != nil {
                RecentsDatabase().addRecentSearch(text: searchText!)
            }
    ...
    // segue to the tweet detail MVC
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "show detail" {
            if let cell = sender as? TweetTableViewCell, let indexPath = tableView.indexPath(for: cell), let destinationController = segue.destination as? TweetDetailTableViewController {
                destinationController.addTweetDetail(tweet: tweets[indexPath.section][indexPath.row])
                destinationController.title = tweets[indexPath.section][indexPath.row].user.name
            }
        }
    }
}
